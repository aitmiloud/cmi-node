...
export class CmiClient implements CmiClientinterface {
  /**
  ...
  constructor(requiredOpts: CmiOptions) {
    ...
    if (!Array.isArray(requiredOpts)) {
      throw new Error('requiredOpts must be an array');
    }
   ...
   */
  generateHash(storekey: string | null | undefined): string {
  ...
  private validateOptions(opts: CmiOptions): void {
    ...
    if (!opts.storekey) {
      throw new Error('storekey is required');
    }
